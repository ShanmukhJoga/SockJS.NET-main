...
namespace syp.biz.SockJS.NET.Client.Implementations.Transports
{
    ...
    internal class SystemWebSocketTransport : ITransport
    {
        ...
        public async Task SendWithTopic(string data)
        {
            ...
            this.VerifyOpen();
            var buffer = Encoding.UTF8.GetBytes(data).AsMemory();
           var resp=  this._socket.SendAsync(buffer, WebSocketMessageType.Text, true, CancellationToken.None);

        //  await  this._socket.SendAsync(buffer, WebSocketMessageType.Text, true, CancellationToken.None);
